...
public protocol ErrorHandling: AnyObject {
    ...
    func bind<Subject>(errorTo subject: Subject, autoCancel: Bool) where Subject: Combine.Subject, Subject.Output == Error, Subject.Failure == Never
}
...
public extension ErrorHandling {
    private var errorType: ErrorType {
        ...
        set {
            objc_setAssociatedObject(self, &errorTypeKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }

    func receiveError<E: Error>(_ type: E.Type = Error.self) -> AnyPublisher<E, Never> {
        guard type.self != Error.self else {
            return errorSubject
                .share()
...
public extension Publisher {
    func catchError<Owner>(in handler: ErrorHandler<Owner>) -> AnyPublisher<Output, Failure> {
        bindError(to: handler.errorSubject)
    }
}

public extension ErrorHandling {
    func catchError<Owner>(in handler: ErrorHandler<Owner>) {
        errorSubject
            .share()
            .bind(to: handler.errorSubject, autoCancel: false)
            .store(in: &cancellables)
    }
}
